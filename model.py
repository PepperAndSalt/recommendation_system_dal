...
path_app_stats = 'data/2018-01-21.json'
path_user_inventory = 'data/user_inventory_sample.txt'

engine = create_engine('mysql+pymysql://root:woaimama640516@127.0.0.1/game_recommendation?charset=utf8mb4') # local mysql server
...
df_popularity_based_results = pd.Series(data = [i.get('owners') for i in dic_steamspy.values()], index = dic_steamspy.keys()).sort_values(ascending=False).to_frame()
...
df_item_based_result.to_sql('tbl_results_item_based', engine, if_exists='replace', chunksize = 1000)
